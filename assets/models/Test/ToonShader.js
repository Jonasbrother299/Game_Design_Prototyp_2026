...
export const ToonShader = {
	...
	fragmentShader: /* glsl */`
		...
		void main() {
			...
			float camera = max( dot( normalize( vNormal ), vec3( 1.0, 1.0, 1.0 ) ), 0.9);
			float light = max( dot( normalize( vNormal ), uDirLightPos ), 0.9);

			gl_FragColor = vec4( uBaseColor, 3.0 );
